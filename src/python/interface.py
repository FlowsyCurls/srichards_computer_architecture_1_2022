...
class Interface:
    ...
    def create_figure(self):
        ...
        # square
        x = y = 2
        self.square = patches.Rectangle((x,y), 93, 93, linewidth=3, edgecolor=self.sq_color, facecolor='none')
        ...
        bn_load = Button(ax_button,'Upload', color=self.bn1_color, hovercolor=self.lines_color)
        bn_load.on_clicked(self.load_image)
        # add apply button
        ax_apply = plt.axes([0.505, 0.03, 0.1, 0.075])
        bn_apply = Button(ax_apply,'Apply',color=self.bn2_color, hovercolor=self.lines_color)
